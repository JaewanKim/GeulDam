			...
			<div class="container">
				<div class="row r-pb">
					<div class="col-md-8 col-md-offset-2 text-center section-heading">
						<h2 > 담은 글</h2><br>
						<p class="fh5co-lead animate-box">내가 담아온 글을 확인할 수 있습니다.</p>
					</div>
				</div>
            </div>
...
try{
	...
	rs=stmt.executeQuery("select* from manager where  userNum='" +whoami+ "'and writingType=0"); // 내가 담은글 type =0;

	while(rs.next()){
		writingNum=rs.getString("writingNum");
		System.out.println(writingNum);
...
try{
	...
	conn = DriverManager.getConnection(URL, USER, PASS);
	stmt =conn.createStatement();
	//int whoami = Integer.parseInt(request.getParameter("userNum"));
	rs=stmt.executeQuery("select writingContext from writing where  writingNum="+writingNum);

	while(rs.next()){
		String context=rs.getString("writingContext");
		%>
		<article class="white-panel" >
        <a class="viewingModalPopup" href="writingmodal.jsp?num=<%=writingNum%>">
   			<%= context %>
        </article>
<% 
	}
